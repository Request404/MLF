    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                #     m.conv = fuse_conv_and_bn(m.pc3, m.bn)  # update conv
                #     delattr(m, "bn")  # remove batchnorm
                #     m.forward = m.forward_fuse  # update forward
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                # else:
                args.insert(2, n)  # number of repeat
                if m in {MLF}:
                    args[2] = n if depth >= 1 else min(2, n)
                n = 1

            if m in {C3k2} and scale in "mlx":  # for M/L/X sizes
                args[3] = True
        elif m is AIFI:
            args = [ch[f], *args]
...
def yaml_model_load(path):
    ...
    path = Path(path)
    print(path, "cccc")
    # Skip P6 renaming and unified_path logic for MLF models
    if "mlf" not in path.stem.lower():  # 如果是非 MLF 模型，才执行原有逻辑
        print(path, "ssss")
        ...
        unified_path = re.sub(r"(\d+)([nslmx])(.+)?$", r"\1\3", str(path))  # yolov8x.yaml -> yolov8.yaml
        yaml_file = check_yaml(unified_path, hard=False) or check_yaml(path)
    else:  # 如果是 MLF 模型，直接加载原始文件
        print(path,"dddd")
